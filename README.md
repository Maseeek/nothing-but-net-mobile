...
2. **Open the project in Android Studio** (or VS Code with the *Kotlin* plugin). Choose **File → New → Import Project** and point at the `src/` folder once it exists.
3. **Build the app** – run `./gradlew assembleDebug` (or click *Run* in Android Studio). The build should succeed with no source files yet; this validates the Gradle wrapper.
...
2. **Update state** – after finishing a step, run the *state‑update* skill (the agent does this automatically) which appends a short summary to the phase README.
3. **Track tasks** – the *task‑manager* skill updates `TASKS.md` checkboxes based on completed work.
...
*Generated by the Antigravity agent following the ICM workflow.*
...
- **.agents/** – contains the agent’s configuration, rules, and reusable *skills* (`state-update`, `task-manager`). Do not edit these unless you are extending the agent.
...
*For any questions about the workflow, refer to the `.agents/rules/` directory or ask the Antigravity agent.*
